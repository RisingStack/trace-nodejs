...
function TraceReporter (options) {
  this.COLLECTOR_API_SAMPLE = url.resolve(options.collectorApi,
    options.collectorApiSampleEndpoint)
  this.COLLECTOR_API_SERVICE = url.resolve(options.collectorApi,
    options.collectorApiServiceEndpoint)
  this.COLLECTOR_API_METRICS = url.resolve(options.collectorApi,
    options.collectorApiApmMetricsEndpoint)
  this.COLLECTOR_API_RPM_METRICS = url.resolve(options.collectorApi,
  ...
  var req = https.request({
    ...
    res.pipe(bl(function (err, resBuffer) {
      ...
      if (res.statusCode === 401) {
        return console.log('Your TRACE_API_KEY got rejected - are you sure you are using the right one?')
      }
