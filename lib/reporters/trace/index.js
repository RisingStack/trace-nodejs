...
var COLLECTOR_API_METRICS = url.resolve(config.collectorApi,
  config.collectorApiMetricsEndpoint);
var COLLECTOR_API_RESPONSES = url.resolve(config.collectorApi,
  config.collectorApiResponsesEndpoint);
...
TraceReporter.prototype.sendRpm = function (data) {
  var opts = url.parse(COLLECTOR_API_RESPONSES);
  var payload = JSON.stringify(data);
