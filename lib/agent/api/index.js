...
var debug = require('debug')('risingstack/trace:api')
...
CollectorApi.prototype._sendSync = function (destinationUrl, data, options) {
  debug(dbgFormat(dbgFormat.levels.INFO, '_sendSync', 'Sending data to ' + destinationUrl))
  try {
    ...
    debug(dbgFormat(dbgFormat.levels.INFO, '_sendSync', 'Server responded with error: ' + ex))
  }
}
...
CollectorApi.prototype._send = function (destinationUrl, data, callback, options) {
  ...
  debug(dbgFormat(dbgFormat.levels.INFO, '_send', 'Sending data to ' + destinationUrl))

  req.on('error', function (error) {
    debug(dbgFormat(dbgFormat.levels.ERROR, '_send', 'Connection error: ' + error))
    callback(error)
  })
...
CollectorApi.prototype.getService = function (cb) {
  ...
  var req = https.request({
    ...
    res.pipe(bl(function (err, resBuffer) {
      ...
      if (err) {
        debug(dbgFormat(dbgFormat.levels.ERROR, 'getService', err))
        return setTimeout(function () {
          debug(dbgFormat(dbgFormat.levels.WARN, 'getService', format('Retrying with %d ms', retryInterval)))
          _this.getService()
        }, retryInterval)
      }
      ...
      if (res.statusCode === 401) {
        debug(dbgFormat(dbgFormat.levels.ERROR, 'getService', 'Api key rejected'))
        return
      }
      if (res.statusCode > 399) {
        debug(dbgFormat(dbgFormat.levels.ERROR, 'getService', 'Service responded with ' + res.statusCode))
        return setTimeout(function () {
          debug(dbgFormat(dbgFormat.levels.WARN, 'getService', format('Retrying with %d ms', retryInterval)))
          _this.getService(cb)
        }, retryInterval)
  ...
  req.on('error', function (error) {
    debug(dbgFormat(dbgFormat.levels.ERROR, 'getService', error))
  })
  req.write(payload)
  req.end()
}
...
function logServiceKeyError (method) {
  debug(dbgFormat(dbgFormat.levels.ERROR, method, 'Service key not present, cannot send'))
}
