...
Security.prototype.collectSnykFlags = function collectSnykFlags (callback) {
  fs.readFile('.snyk', function (error, data) {
    ...
    try {
      ...
      debug(dbgFormat(dbgFormat.levels.ERROR, 'collectSnykFlags', format('error collecting snyk flags: %s', ex)))
      debug(ex)
      callback(null, [])
      return
    }
    ...
    childProcess.execFile('npm', ['ls'].concat(uniq(packageNames)).concat(['--parseable', '--long']), function (_error, stdout, stderr) {
      if (_error) {
        // ignore
        debug(dbgFormat(dbgFormat.levels.ERROR, 'collectSnykFlags',
...
Security.prototype.sendDependencies = function sendDependencies () {
  ...
  this.collectDependencies(function (error, dependencies) {
    if (error) {
      debug(dbgFormat(dbgFormat.levels.ERROR, 'sendDependencies', format('error collecting dependencies: %s', error)))
      return
    }

    _this.collectSnykFlags(function (_error, flags) {
      if (_error) {
        debug(dbgFormat(dbgFormat.levels.ERROR, 'sendDependencies', format('error collecting snyk flags: %s', _error)))
