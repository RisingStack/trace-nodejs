...
function Agent (options) {
  debug(dbgFormat(dbgFormat.levels.INFO, 'Agent', 'Initializing api client...'))
...
Agent.prototype.start = function () {
  ...
  this._api.getService(function (err, serviceKey) {
    ...
    debug(dbgFormat(dbgFormat.levels.INFO, 'start', 'ServiceKey is set to: ' + serviceKey))
    self.serviceKey = serviceKey
    debug(dbgFormat(dbgFormat.levels.INFO, 'start', 'Starting agents...'))
    self.agents.forEach(function (agent) {
      agent.initialize({ serviceKey: serviceKey })
      agent.start()
      debug(dbgFormat(dbgFormat.levels.INFO, 'start', agent.name + ' started'))
    })
  })
}

Agent.prototype.stop = function (callback) {
  debug(dbgFormat(dbgFormat.levels.INFO, 'stop', 'Stopping agents...'))
