...
Collector.prototype.onServerSend = function (data) {
  ...
  if (this.partials[data.id] && this.partials[data.id].events) {
    hasStackTrace = this.partials[data.id].events.some(function (event) {
      return event.type === 'st';
    });

    return delete this.partials[data.id];
  }
