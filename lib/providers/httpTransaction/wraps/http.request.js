...
function wrapRequest (originalHttpRequest, collector, config, mustCollectStore) {
  ...
  return function wrappedRequest (requestParams) {
    ...
    if (mustCollectStore[requestId]) {
      debug(format('trace event (cs); reqId: %s, spanId: %s must collect', requestId, spanId))
    ...
    returned.on('error', function (err) {
      debug(format('trace event (cr) on error; reqId: %s, spanId: %s must collect', requestId, spanId))
    ...
    returned.on('response', function (incomingMessage) {
      ...
      if (mustCollectStore[requestId]) {
        debug(format('trace event (cr) on response; reqId: %s, spanId: %s must collect', requestId, spanId))
      }
