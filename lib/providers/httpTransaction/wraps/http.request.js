...
function wrapRequest (originalHttpRequest, collector, config, mustCollectStore) {
  ...
  return function wrappedRequest (requestParams) {
    ...
    debug('trace event (cs); reqId: %s, spanId: %s, requestParams: %s',
      requestId, spanId, requestParams)
    ...
    returned.on('response', function (incomingMessage) {
      ...
      if (mustCollectStore[requestId]) {
        debug('trace event (cr) on response; reqId: %s, spanId: %s must collect, with params %s', requestId, spanId, requestParams)
      } else {
        debug('trace event (cr) on response; reqId: %s, spanId: %s, with params %s', requestId, spanId, requestParams)
      }
