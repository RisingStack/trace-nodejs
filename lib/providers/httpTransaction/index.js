...
HttpTransaction.prototype.startCollecting = function () {
  ...
  this.httpResponsesIntervalId = setInterval(function () {
    _this._sendRpm();
  }, 60 * 1000);
};
...
HttpTransaction.prototype.stopCollecting = function () {
  ...
  clearInterval(this.httpResponsesIntervalId);
};
...
HttpTransaction.prototype.onServerSend = function (data) {
  ...
  if (!this.responses[trace.statusCode]) {
    this.responses[trace.statuCode] = 1;
  } else {
    this.responses[trace.statuCode]++;
  }
...
HttpTransaction.prototype._sendRpm = function () {
  debug('sending responses to the trace service');
  if (Object.keys(this.responses).length > 0) {
    var dataBag = {};
    dataBag.requests = cloneDeep(this.responses);
    dataBag.timestamp = new Date().toISOString();

    this.eventBus.emit(this.eventBus.HTTP_RESPONSES, dataBag);
    this.responses = {};
  }
};
