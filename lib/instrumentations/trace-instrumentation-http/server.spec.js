...
var IncomingEdgeMetrics = require('../../agent/metrics/incomingEdge/index')
...
describe('The http.Server.prototype wrapper module', function () {
    ...
    agent = {
      ...
      tracer: {
        collector: {
          ...
          defaultSeverity: 3,
          bindToHistory: this.sandbox.spy(function (history, cb) {
            return cb
          }),
  ...
  it('new storage session is bound', function () {
    this.sandbox.stub(microtime, 'now').returns(5)
    var s = server(original, agent)

    agent.tracer.collector.bindToHistory = this.sandbox.spy(function (h, cb) { return cb })
    ...
    expect(agent.tracer.collector.bindToHistory).to.be.called
  })
