...
var debug = require('debug')('risingstack/trace:instrumentation')
...
function loadInstrumentationsForTarget (name, path) {
  ...
  try {
    ...
    debug(dbgFormat(dbgFormat.levels.WARN, 'loadInstrumentationsForTarget',
  ...
  libraryTargets.forEach(function (target) {
    target.instrumentations.forEach(function (inst) {
      ...
      if (instrumentationLookup[inst.path]) {
        return debug(dbgFormat(dbgFormat.levels.WARN, 'buildInstrumentationIndex',
          format('[Warning] multiple instrumentations found for path %s. Overriding previous', inst.path)))
      }
...
function instrumentModule (target, instrumentation, agent, load) {
  ...
  if (target.package) {
    try {
      ...
      debug(dbgFormat(dbgFormat.levels.WARN, 'instrumentModule', format('[Warning] cannot load `[%s] package.json`, ' +
  ...
  if (instrumentation.pre) {
    try {
      ...
      debug(dbgFormat(dbgFormat.levels.INFO, 'instrumentModule', format('Preload hook applied to module `[%s] %s`',
        target.name, instrumentation.path)))
    } catch (err) {
      debug(dbgFormat(dbgFormat.levels.ERROR, 'instrumentModule', format(
  ...
  if (loadedModule && instrumentation.post) {
    try {
      ...
      debug(dbgFormat(dbgFormat.levels.INFO, 'instrumentModule', format('Postload hook applied to module `[%s] %s`',
        target.name, instrumentation.path)))
    } catch (err) {
      debug(dbgFormat(dbgFormat.levels.ERROR, 'instrumentModule', format(
    ...
    target.instrumentations.forEach(function (instrumentation) {
      if (instrumentation.path) {
        try {
          ...
          debug(dbgFormat(dbgFormat.levels.ERROR, 'loadTargetsEagerly', format('[Warning] Could not load module `[%s] %s`: `%s`',
            target.name, instrumentation.path, err.message)))
        }
      }
    })
  ...
  shimmer.wrap(Module, '_load', function (load) {
    return function (file, parent) {
      ...
      if (lookupEntry && !lookupEntry.wrapped) {
        ...
        debug(dbgFormat(dbgFormat.levels.INFO, 'create', format('successfully instrumented `[%s] %s`',
