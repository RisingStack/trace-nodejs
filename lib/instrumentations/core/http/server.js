...
function httpServer (listener, agent) {
  ...
  return function (request, response) {
    ...
    debug(dbgFormat(dbgFormat.levels.INFO, 'httpServer', format('SR(%s) [%s %s %s %s] [must-collect: %s]',
    ...
    Shimmer.wrap(response, 'writeHead', function wrapWriteHead (original, name) {
      return function onWriteHead () {
        if (writeHeadAlreadyCalled === false) {
          ...
          if (!tracerBriefcase) {
            debug(dbgFormat(dbgFormat.levels.WARN, 'httpServer',
          ...
          debug(dbgFormat(dbgFormat.levels.INFO, 'httpServer', format('SS(%s) [%s %s %s] [must-collect: %s]',
          ...
          if (!ss.error) {
            ...
            debug(dbgFormat(dbgFormat.levels.WARN, 'httpServer', 'failed to create SS, cannot intrument headers'))
          }
          agent.rpmMetrics.addResponseTime(ssTime - srTime)
          agent.rpmMetrics.addStatusCode(statusCode)
        } else {
          debug(dbgFormat(dbgFormat.levels.WARN, 'httpServer', 'writeHead called multiple times, wrongly reported status codes may occur'))
        }
