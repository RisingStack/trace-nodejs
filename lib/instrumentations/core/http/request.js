...
function httpClient (originalHttpRequest, agent) {
  ...
  return function wrappedRequest (requestParams) {
    ...
    debug(dbgFormat(dbgFormat.levels.INFO, 'httpClient', format('CS(%s) [%s %s %s %s] [must-collect: %s]',
    ...
    returned.on('error', function (err) {
      debug(dbgFormat(dbgFormat.levels.INFO, 'httpClient',
        format('NE(%s) [%s]', csCtx.communicationId, err.code)))
    ...
    returned.on('response', function (incomingMessage) {
      ...
      debug(dbgFormat(dbgFormat.levels.INFO, 'httpClient', format('CR(%s) [%s %s %s] [severity: %s]', csCtx.communicationId,
