...
var wrapper = require('./trace-instrumentation-mysql')
...
describe('The mysql wrapper', function () {
  ...
  it('should use wrapQuery with expected arguments to wrap connection.query with callback', function (done) {
    var sandbox = this.sandbox
    var fakeWrapQuery = sandbox.stub(utils, 'wrapQuery')
    var fakeAgent = { clearly: 'a mock' }
    ...
    var shimmerWrapStub = sandbox.stub(Shimmer, 'wrap').callsFake(function (nodule, name, cb) {
      expect(name).to.eql(CONNECTION_OPERATIONS)
      var queryStr = 'SELECT 1 + 1 AS solution'
      var queryArguments = [queryStr, function () {}]
      var fakeQuery = function () {}
      cb(fakeQuery, 'query').apply(nodule, queryArguments)
      expect(fakeWrapQuery).to.have.been.calledWith(
        fakeQuery,
  ...
  it('should use wrapQuery with expected arguments to wrap pool.query with callback', function (done) {
    var sandbox = this.sandbox
    var fakeWrapQuery = sandbox.stub(utils, 'wrapQuery')
    var fakeAgent = { clearly: 'a mock' }
