...
function parseNumberOrUndefined (string) {
  if (!string) { return }
...
function consumerWrapper (agent, original) {
  return function (queue, callback, options, cb0) {
    ...
    var wrapped = function (msg) {
      ...
      var parentServiceKey = parseNumberOrUndefined(get(msg, 'properties.headers[\'x-parent\']'))
      var timestamp = microtime.now()
      var transportDelay = isNaN(originTimestamp) ? undefined : timestamp - originTimestamp
      debug(dbgFormat(dbgFormat.levels.INFO, 'amqplib.consumerWrapper',
        format('incomingEdge [%s %s %s]', 'amqp', parentServiceKey, transportDelay)))
...
function sendMessageWrapper (agent, original) {
  return function (channel, Method, fields, Properties, props, content) {
    ...
    debug(dbgFormat(dbgFormat.levels.INFO, 'amqplib.sendMessageWrapper',
      'sending instrumented message...'))
